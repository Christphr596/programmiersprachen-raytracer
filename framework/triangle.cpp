...
HitPoint Triangle::intersect(Ray const& ray) {
	...
	float p = o.x * ray.direction.x + o.y * ray.direction.y + o.z * ray.direction.z; //Skalarprodukt aus Kreuzprodukt und Richtungsvektor
	float value = o.x * point1_.x + o.y * point1_.y + o.z * point1_.z; //Ebenengleichung Wert (=value)


	if(p != 0){
		float d = ((o.x * ray.origin.x + o.y * ray.origin.y + o.z * ray.origin.z) / (-o.x * ray.direction.x - o.y * ray.direction.y - o.z * ray.direction.z)) - value; //Werte	
		glm::vec3 cut_e = ray.origin + d * ray.direction;
